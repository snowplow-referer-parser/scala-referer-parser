...
class ParseTest extends Specification {
  def is = s2"""
  Parser.parse should
    return the expected result for each of the specified referers $e1
  """
  ...
  private def test(refererSpec: RefererSpec) =
    refererSpec match {
      case RefererSpec(refererUri, medium, source, term) =>
        (ioParser.parse(refererUri, pageHost) ==== Some(genExpected(medium, source, term))) and
  ...
  def genExpected(
    ...
    (medium, source) match {
      case ("internal", _) => InternalReferer
      case ("unknown", _)  => UnknownReferer
      case (_, Some(src))  => ExternalReferer(medium, src, term)
      case _               => UnknownReferer
    }
}
